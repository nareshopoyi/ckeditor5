...
function getTextNode(position){
	return position.textNode  || position.nodeBefore;
}
...
export default class MentionCommand extends Command {
	...
	execute( options ) {
		...
		model.change( writer => {
			...
			// console.log(attributesWithMention,getTextNode(model.document.selection.focus),
			// Array.from( getTextNode(model.document.selection.focus).getAttributes()),
			// hasExistingMention( model.document.selection.focus),
			// Array.from(range));

			//console.log(this.editor.model.markers.getMarkersAtPosition())
			if ( hasExistingMention( model.document.selection.focus ) == false ) {
				//getTextNode(model.document.selection.focus)
			...
			// const end_ = model.createPositionAt( model.document.selection.focus );
			// const start_ = model.createPositionAt( mentionMarker.getStart() );
			// const range_ = model.createRange( start_, end_ );
			const textNode = getTextNode(model.document.selection.focus);
			const sellection_ = writer.createSelection( textNode, 'on' );
			model.deleteContent(sellection_)

			model.insertContent( writer.createText( mentionText, attributesWithMention));
				//writer.setSelection(getTextNode(model.document.selection.focus),'on');
				//writer.setSelectionAttribute( new Map( [ [ 'italic', true ] ] ));
				//writer.removeAttribute( 'mention', textNode );
				//writer.setAttribute( 'mention', attributesWithMention, textNode );
				//writer.insertText(mymentionText,attributesWithMention,'after')
