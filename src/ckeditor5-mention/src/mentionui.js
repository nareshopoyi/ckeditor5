...
export default class MentionUI extends Plugin {
	...
	init() {
			...
			(evt, data) => {
				if (isHandledKey(data.keyCode) && this._isUIVisible) {
					...
					if(data.keyCode == keyCodes.space){
						try{
						if(this._lastRequested.toLocaleLowerCase()===this._mentionsView.selected.item.name.toLocaleLowerCase()){
		...
		for (const mentionDescription of feeds) {
			...
			const watcher = this._setupTextWatcherForFeed(
				marker,
				minimumCharacters
			);
	...
	_setupTextWatcherForFeed(marker, minimumCharacters) {
		...
		watcher.on('matched', (evt, data) => {
			...
			// 	data.range.end = data.range.end.getShiftedBy(-1)
			...
			const range = model.createRange(start_, end_);

			if (marker === '#') {
				editor.execute('mention', {
					mention: {
						id: marker + feedText,
						link: '/topic/' + feedText.toLowerCase(),
						name: feedText,
						postCount: 0,
						userId: '',
					},
					text: marker + feedText,
					marker,
					range,
				});
			}
	...
	_handleFeedResponse(data) {
		const { feed, marker, feedText } = data;
		...
		if (this._items.length) {
			...
			if (
				marker === '#' &&
				feedText.toLocaleLowerCase() === feed[0].name.toLocaleLowerCase()
			) {
				const mentionMarker = this.editor.model.markers.get('mention');
				const end_ = this.editor.model.createPositionAt(this.editor.model.document.selection.focus);
				const start_ = this.editor.model.createPositionAt(mentionMarker.getStart());
				const range = this.editor.model.createRange(start_, end_);
